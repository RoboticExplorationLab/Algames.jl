...
function build_robot!(vis::Visualizer, model::QuadrotorGame; name::Symbol=:Robot, r_col=0.08, α=1.0, r=0.15)
	r = convert(Float32, r)
    p = model.p

	obj_path = joinpath(@__DIR__, "..", "mesh", "quadrotor", "drone.obj")
	mtl_path = joinpath(@__DIR__, "..", "mesh", "quadrotor", "drone.mtl")
	orange_mat, blue_mat, black_mat = get_material(;α=0.3)
	obj = MeshFileObject(obj_path)
    for i = 1:p
		setobject!(vis[name]["player$i"]["body"], obj)
		setobject!(vis[name]["player$i"]["collision"], GeometryBasics.Sphere(Point3f0(0.0), r_col), black_mat)
