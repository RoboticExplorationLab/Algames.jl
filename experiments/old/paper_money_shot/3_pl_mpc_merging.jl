...
x0 = @SVector [
               ...
               -1.00, -0.04,  0.00, 0.60, #lane2
               -0.90, -0.30, pi/12, 0.63, #lane4
                ]
xf = @SVector [
                1.10, -0.05,  0.00, 0.62, #lane1
                0.70, -0.04,  0.00, 0.60, #lane2
                0.90, -0.06,  0.00, 0.61, #lane4
               ]
...
@time solve!(solver_directgames)
full_reset!(solver_directgames)
opts_mpc_gamessolver = MPCGamesSolverOptions{T}(iterations=500, mpc_tf=3.0)
...
solve!(mpc_solver, state_noise, selfish_inds, selfish_dx;
    min_δt=0.00, display=false, wait=true)

animation(mpc_solver, scenario, plx; display_actors=true)



i = mpc_solver.stats.iterations
m = mean(mpc_solver.stats.solve_time[1:i])
