...
opts_monte_carlo = MonteCarloSamplerOptions{T}(
    noise=0.03,
    iterations=10)
sampler = MonteCarloSampler(solver_directgames, opts_monte_carlo)
monte_carlo_sampling(sampler)

include("../../src/utils/monte_carlo_visualization.jl")
visualize_H_cond(sampler)
visualize_solve_time(sampler)
visualize_cmax(sampler)
visualize_iterations_total(sampler)
visualize_optimality_merit(sampler)
...
plt = plot()
histogram!(sampler.stats.iterations_total)
