...
x0 = @SVector [
               -0.60, -0.15,  0.00, 0.60, #lane1
                1.20,  0.15,  pi, 0.60, #lane2
...
@time solve!(solver_directgames)
full_reset!(solver_directgames)
opts_mpc_gamessolver = MPCGamesSolverOptions{T}(iterations=30)
...
selfish_dx = @SVector [0., 0., 0., 0.]
# solve!(mpc_solver, state_noise; min_δt=0.0, selfish_inds=zeros(Int,0), selfish_dx=SVector{0}([]))
