...
vis, anim = plan_animation(solver_directgames, scenario, plx;
...
X_mpc = TO.states(mpc_solver)[10:end]
iter = mpc_solver.stats.iterations
times = cumsum(mpc_solver.stats.solve_time[1:iter])
times .-= times[1]
M = 400
X_sam = [X_mpc[1]]
for k = 2:M-1
    t_sam = (k-1)*times[end]/M
    ind = index_next(t_sam, times)
    x_prev = X_mpc[ind-1]
    x_next = X_mpc[ind]
    step = (t_sam - times[ind-1])/(times[ind] - times[ind-1])
    x_interp = x_prev .+ (x_next .- x_prev)*step
    push!(X_sam, x_interp)
end
push!(X_sam, X_mpc[end])



function index_next(t, times)
...
for k = 1:N
    u = TO.control(solver_directgames.Z[k])
    solver_directgames.Z[k].z = [X_sam[k]; u]
end
