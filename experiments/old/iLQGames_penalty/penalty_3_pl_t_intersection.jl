...
x0 = @SVector [
               -0.70, -0.15,  0.00,  0.00, #lane1 player 2 goes first
                0.70,  0.15,  0.00,  0.00, #lane2 player 2 goes first
                0.15, -0.70,  0.00,  0.70, #lane2 player 2 goes first
                ]
xf = @SVector [
               -0.70, -0.15,  0.00,  0.00, #lane1
                0.70,  0.15,  0.00,  0.00, #lane1
               # -1.20,  0.15, -1.10,  0.00, #lane1
                0.15, -0.70,  0.00,  0.00, #lane1
               ]
dxf = @SVector [
                0.70,  0.00,  0.00,  0.00, #lane1
               -0.70,  0.00,  0.00,  0.00, #lane2
               -0.00,  0.70,  0.00,  0.00, #lane2
               ]
...
# obs = [0.0, -0.20]
scenario = TIntersectionScenario(road_length,
    road_width, bot_length, cross_width, car_radii, bound_radius)
lanes = [1,2,3]
# Collision Avoidance
conSet = ConstraintSet(n,m,N)
add_collision_avoidance(conSet, car_radii, plx, p)
add_scenario_constraints(conSet, scenario, lanes, plx, N, n; constraint_type=:constraint)
