...
x0 = @SVector [
               ...
               # 0.43, -0.30,  pi/2, 0.10, #lane4 # slower
               0.43, -0.30,  pi/2, init_speed, #lane4 # quicker
                ]
xf = @SVector [
              ...
              # -0.60,  0.15,  pi, 0.80, #lane4
               0.43,  0.30,  pi/2, expected_speed, #lane4
               ]
...
dxf = @SVector [
              ...
              # -0.60,  0.00,  0.00,  0.00, #lane2
               0.00,  real_speed,  0.00,  0.00, #lane2
              ]
...
scenario = TIntersectionScenario(
    top_road_length, road_width, bottom_road_length, cross_width, car_radii, bound_radius)
...
@time solve!(solver_directgames)
full_reset!(solver_directgames)
opts_mpc_gamessolver = MPCGamesSolverOptions{T}(iterations=150)
...
# selfish_dx = @SVector [0., 0.1, 0., 0.] # slower than the expected behavior
selfish_dx = @SVector [0., real_speed, 0., 0.] # quicker than the expected behavior
solve!(mpc_solver, state_noise, selfish_inds, selfish_dx; min_δt=0.02)
# solve!(mpc_solver, state_noise; min_δt=0.01, selfish_inds=selfish_inds, selfish_dx=selfish_dx)
