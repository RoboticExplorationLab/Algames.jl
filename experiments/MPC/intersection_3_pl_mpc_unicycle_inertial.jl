...
function dynamics(model::InertialUnicycleGame, x, u)
    ...
    qd1 *= x[4]
    qd2 = @SVector [cos(x[6]), sin(x[6])]
    qd2 *= x[8]
    qd3 = @SVector [cos(x[9]), sin(x[9])]
    qd3 *= x[12]
    qdd1 = u[ @SVector [1,2] ]
    qdd2 = u[ @SVector [3,4] ]
    qdd3 = u[ @SVector [5,6] ]
...
x0 = @SVector [
               -0.15,  0.3,  -pi/2, 1.0, #lane1
                0.4,   0.15,  pi,   1.0, #lane2
               -0.9,  -0.15,  0.0,  1.0, #lane4
                ]
xf = @SVector [
                0.6,  -0.15, 0.0,  1.0, #lane1
               -0.15, -0.50,-pi/2, 1.0, #lane2
               # -0.15 ,  0.3,  0.0, 1.0, #lane4
                0.15,  0.20, pi/2, 1.0, #lane4
               ]
