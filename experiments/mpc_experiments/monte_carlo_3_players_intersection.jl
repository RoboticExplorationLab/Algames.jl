...
include("algames_solver_3_players_intersection.jl")
# include("approx_solver_3_players.jl")
# include("const_vel_solver_3_players.jl")
...
state_noise = 5. * SVector{n}([
	0.008,   0.008,   0.008,
	0.008,   0.008,   0.008,
	2*pi/72, 2*pi/72, 2*pi/72,
	0.03,    0.03,    0.03]) #+-50cm, +-50cm, +-25deg, +-12.5% per second
...
mpc_state_noise = 0* SVector{n}([
	0.50,   0.50,    0.50,  # 20m
	0.02,   0.02,    0.02,  # 40cm
	pi/30,  pi/30,   pi/30, # 6 deg
	0.05,   0.05,    0.05]) # 1m/s
iter = 10
...
vis, anim = animation(algames_mpc_solver, intersection_3_players_unicycle_penalty_scenario;
	vis=vis, anim=anim,
	open_vis=false,
	display_actors=true,
	display_trajectory=true)

vis, anim = animation(approx_mpc_solver, intersection_3_players_unicycle_penalty_scenario;
	vis=vis, anim=anim,
	open_vis=false,
	display_actors=true,
	display_trajectory=true)
...
vis, anim = animation(const_vel_mpc_solver, ramp_merging_3_players_unicycle_penalty_scenario;
	vis=vis, anim=anim,
	open_vis=false,
	display_actors=true,
	display_trajectory=true)
