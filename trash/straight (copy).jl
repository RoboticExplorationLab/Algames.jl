...
function build_scenario(vis::Visualizer, scenario::StraightScenario{T}) where T
	...
	obs_radius = scenario.obs_radius
	obs = scenario.obs

	actor_radius = actors_radii[1][end]
	# Check all radii are the same
	check = all([actors_radius[end] for actor_radius in actors_radii] .== actor_radius) ### only check the last radius
	if !check
