	...
	for i = 1:length(actors_radii)
		if actors_types[i] == :car
			...
			collision_cylinder = Cylinder(Point(0.0,0.0,0.0),
				Point(0.0,0.0,cylinder_height_car), actors_radii[i][end]) ### need to plaot cylinders per timestep
		...
		elseif actors_types[i] == :pedestrian
			...
			# Add collision avoidance cylinders
			collision_cylinder = Cylinder(Point(0.0,0.0,0.0),
				Point(0.0,0.0,cylinder_height_ped), actors_radii[i][end]) ### need to plaot cylinders per timestep
