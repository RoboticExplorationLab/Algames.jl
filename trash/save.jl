...
# struct DirectGamesSolver{T,I<:QuadratureRule,L,O,n,n̄,m,m1,L1,L2,D,F,E1,E2} <: ConstrainedSolver{T}
struct DirectGamesSolver{T,I<:QuadratureRule,L,O,n,m,m1,L1,D,E1} <: ConstrainedSolver{T}
    # Model + Objective
    model::L
    obj::Vector{O}

    spu::Vector{SVector{m1,Int}}

    xinds::Vector{SVector{n,Int}}
    uinds::Vector{SVector{m,Int}}
    xinds_p::Vector{Vector{SVector{n,Int}}}
    uinds_p::Vector{Vector{SVector{m1,Int}}}
    uinds_H::Vector{Vector{SVector{m1,Int}}}
    ...
    function DirectGamesSolver{T,I}(model::L, obj::Vector{O},
            ...
            ν_::Vector{Vector{SVector{n,T}}},
            γ::Vector{Vector{SVector{n,T}}},
            logger) where {T,I,L,O,n,m,m1,L1,D,E1}
        new{T,I,L,O,n,m,m1,L1,D,E1}(model, obj,
            spu,
            xinds,
            uinds,
            xinds_p,
...
function DirectGamesSolver(prob::GameProblem{I,T}, opts=DirectGamesSolverOptions{T}()) where {I,T}
    ...
    # Static Array Indices for the control inputs of each player.
    spu = [SVector{length(pu[i])}(pu[i]) for i in 1:p]

    xinds = [-n+(k-1)*(m+n) .+ SOneTo(n) for k=1:N] #okkk
    xinds_p = [[-n*p+(k-1)*(m+n*p)+(i-1)*n .+ SOneTo(n) for k=1:N] for i=1:p] #okkk
    uinds = [(k-1)*(m+n) .+ SOneTo(m) for k=1:N] #okkk
    uinds_p = [[(k-1)*(m+n*p) .+ spu[i] for k=1:N] for i=1:p] #okkk
    uinds_H = [[(k-1)*(m+n) .+ spu[i] for k=1:N] for i=1:p] #okkk
    off = (n+m)*(N-1)
    νinds = [[off+(k-1)*n*p+(i-1)*n .+ SOneTo(n) for k=1:N-1] for i=1:p]
    off_p = (n*p+m)*(N-1)
    νinds_p = [off_p+(k-1)*n .+ SOneTo(n) for k=1:N-1]
    ...
    state_view_H = [[view(H_,xinds_p[i][k_1],xinds[k_1]) for k_1=2:N] for i=1:p]
...
#
# function full_reset!(solver::DirectGamesSolver{T}, reset_stats=true) where T
#     n,m,N = size(solver)
#     if reset_stats
...
Base.size(solver::DirectGamesSolver{T,I,L,O,n,m,m1,L1,D,E1}) where {T,I,L,O,n,m,m1,L1,D,E1} = n,m,solver.N
